...
enum TechnicalAnalysis {
    ...
    // MARK: - RSI Dizisi — her bar için tek değer döndürür (rsi() ile aynı Wilder yöntemi)
    static func rsiArray(closes: [Double], period: Int = 14) -> [Double] {
        guard closes.count > period else {
            return Array(repeating: 50.0, count: closes.count)
        }
        var results = Array(repeating: 50.0, count: period)
        var avgGain = 0.0, avgLoss = 0.0
        for i in 1...period {
            let diff = closes[i] - closes[i - 1]
            if diff > 0 { avgGain += diff } else { avgLoss -= diff }
        ...
        avgGain /= Double(period)
        avgLoss /= Double(period)
        results.append(avgLoss > 0 ? 100 - (100 / (1 + avgGain / avgLoss)) : 100)
        for i in (period + 1)..<closes.count {
            let diff = closes[i] - closes[i - 1]
            avgGain = (avgGain * Double(period - 1) + max(diff, 0))  / Double(period)
            avgLoss = (avgLoss * Double(period - 1) + max(-diff, 0)) / Double(period)
