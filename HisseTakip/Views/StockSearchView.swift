...
struct StockSearchView: View {
    ...
    @State private var directAnalysisStock: Stock? = nil
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationDestination(item: $directAnalysisStock) { stock in
    ...
    private var notFound: some View {
        VStack(spacing: 20) {
            ...
            Image(systemName: "magnifyingglass")
                ...
                .foregroundStyle(.tertiary)
            VStack(spacing: 6) {
                Text("\"\(query.uppercased())\" listede yok")
                    .font(.title3.weight(.bold))
                Text("BIST listesinde bulunamadı — yine de doğrudan analiz edebilirsin.")
                    .font(.subheadline).foregroundStyle(.secondary)
                    .multilineTextAlignment(.center)
            }
            Button { navigateOnSubmit() } label: {
                HStack(spacing: 8) {
                    Image(systemName: "chart.xyaxis.line")
                        .font(.system(size: 15, weight: .semibold))
                    Text("\"\(query.uppercased())\" Analiz Et")
                        .font(.system(size: 15, weight: .semibold))
                }
                ...
                .clipShape(Capsule())
                .shadow(color: Color(red: 0.2, green: 0.5, blue: 1.0).opacity(0.35), radius: 8, y: 4)
            }
            Spacer()
        }
    ...
    // MARK: - Direkt Sembol Analizi

    private func navigateOnSubmit() {
        focused = false
        if let first = results.first {
            addRecent(first)
            directAnalysisStock = first
        } else if !query.trimmingCharacters(in: .whitespaces).isEmpty {
            let sym = query.uppercased().trimmingCharacters(in: .whitespaces)
            directAnalysisStock = Stock(id: "\(sym).IS", symbol: sym, name: sym, sector: "—")
        }
    }
